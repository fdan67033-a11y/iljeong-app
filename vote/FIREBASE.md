...
## 관리자 계정 설정 (한 번만) — 구글 로그인

단계 전환 버튼(admin.html)은 지정한 구글 계정으로 로그인해야 활성화됩니다.
(비밀번호 없이, 프로젝트를 만든 구글 계정으로 바로 로그인)

1. 콘솔 → **Authentication** → 로그인 방법 → **Google** 사용 설정(지원 이메일 선택) → 저장
...
이메일/uid는 공개돼도 안전하고, 실제 인증은 구글이 담당합니다. 비밀번호는 다루지 않습니다.
